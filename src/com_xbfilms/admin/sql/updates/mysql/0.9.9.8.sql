...
ALTER TABLE `#__xbfilms` CHANGE `acq_date` `acq_date` DATE NULL DEFAULT CURRENT_DATE;
#drop acq_date on next update
...
UPDATE `#__xbfilms` SET `first_seen` = `last_seen` WHERE ISNULL(`first_seen`);
...
ALTER TABLE `#__xbfilms` DROP `acq_date`;