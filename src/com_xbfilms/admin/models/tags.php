...
class XbfilmsModelTags extends JModelList {
	...
	protected function getListQuery() {
		...
		$query->where("m.type_alias IN ('com_xbfilms.film','com_xbpeople.person','com_xbfilms.review','com_xbpeople.character')");
		$query->select('(SELECT COUNT(*) FROM #__contentitem_tag_map AS tb WHERE tb.tag_id = t.id AND tb.type_alias='.$db->quote('com_xbfilms.film').') AS bcnt');
		...
		$query->select('(SELECT COUNT(*) FROM #__contentitem_tag_map AS tr WHERE tr.tag_id = t.id AND tr.type_alias='.$db->quote('com_xbfilms.review').') AS rcnt');
		...
		$query->select('(SELECT COUNT(*) FROM #__contentitem_tag_map AS toth WHERE toth.tag_id = t.id AND toth.type_alias NOT IN 
            ('.$db->quote('com_xbfilms.film').','.$db->quote('com_xbfilms.review').','.$db->quote('com_xbpeople.person').','.$db->quote('com_xbpeople.character').'))
             AS othcnt');
