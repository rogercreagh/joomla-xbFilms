...
class XbfilmsControllerImportexport extends JControllerForm
{
	function export() {
		...
		$link = 'index.php?option=com_xbfilms&view=importexport&format=raw';
	...
	function import() {
		...
		$post   = $jinput->get('jform', 'array()', 'ARRAY');
		$model = $this->getModel('importexport');
		$link = 'index.php?option=com_xbfilms&view=importexport';
		...
		if ($wynik['donecnt'] > 0 ) {
			$msg='Data imported. ';
			if ($wynik['#__xbfilms']>0) { $msg .= $wynik['#__xbfilms'].' films, ';}
			if ($wynik['#__xbfilmreviews']>0) { $msg .= $wynik['#__xbfilmreviews'].' reviews, ';}
			if ($wynik['#__xbpersons']>0) { $msg .= $wynik['#__xbpersons'].' people. ';}
			$msg .= ' All assigned to   category.';
			if ($wynik['#__xbfilmperson']>0) { $msg .= $wynik['#__xbfilmperson'].' people-film links created, ';}
